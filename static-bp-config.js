...
async function displayConfigTable(configs) {
  ...
  for (const [filePath, config] of Object.entries(configs)) {
    if (config.JITO?.STATIC_TIP_BP) {
      console.log(
        `| ${index} | ${filePath} | STATIC_TIP_BP | ${config.JITO.STATIC_TIP_BP} |`
      );
      index++;
    }
  }
  ...
  try {
    ...
    if (modifyAll.toLowerCase() === "y") {
      ...
      for (const [filePath, config] of Object.entries(configs)) {
        ...
        currentIndex++;
        if (
          config.TEMPORAL?.TEMPORAL_DYNAMIC_FEE_BP &&
        ...
        currentIndex++;
        if (
          config.FAST?.FAST_DYNAMIC_FEE_BP &&
          currentIndex === selectedIndex
        ) {
          await writeYamlFile(
            filePath,
        ...
        currentIndex++;
        if (
          config.NEXTBLOCK?.NEXTBLOCK_DYNAMIC_FEE_BP &&
          currentIndex === selectedIndex
        ) {
          await writeYamlFile(
            filePath,
        ...
        currentIndex++;
        if (
          config.BLOXROUTE?.BLOXROUTE_DYNAMIC_FEE_BP &&
          currentIndex === selectedIndex
        ) {
          await writeYamlFile(
            filePath,
