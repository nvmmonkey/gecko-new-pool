    ...
    try {
        ...
        for (let i = 0; i < filteredData.nodes.length; i += BATCH_SIZE) {
            ...
            const batchPromises = batch.map(async (node) => {
                const rpcEndpoint = node.rpc.includes(':') ? node.rpc : `${node.rpc}:8899`;
                process.stdout.write(`\rTesting batch ${Math.floor(i/BATCH_SIZE) + 1}/${Math.ceil(filteredData.nodes.length/BATCH_SIZE)}...`);
        ...
        const outputData = {
            ...
            averageLatency: validNodes.length > 0 
                ? validNodes.reduce((acc, node) => acc + node.latency, 0) / validNodes.length 
                : 0,
            lastTested: new Date().toISOString()
        };
