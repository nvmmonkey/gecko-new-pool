...
create_filtered_market_cache() {
    local excluded_dexes="$1"
    local original_file="mainnet.json"
    local filtered_file="mainnet-filtered.json"
    local backup_file="mainnet.json.backup"
  ...
  if [ "$DISABLE_LOCAL_JUPITER" != "true" ]; then
    ...
    if [ "$JUPITER_URL" = "http://0.0.0.0:$LOCAL_JUPITER_PORT" ] || \
      ...
      if [ "$USE_LOCAL_MARKET_CACHE" = "true" ]; then
          echo "Using local market cache from mainnet-filtered.json"
          MARKET_CACHE_PARAM="--market-cache mainnet-filtered.json"
      fi
