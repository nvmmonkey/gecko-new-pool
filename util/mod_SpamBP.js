...
async function displayConfigTable(configs, selectedOption) {
  ...
  for (const [filePath, config] of Object.entries(configs)) {
    if (config.JITO?.[selectedOption] || 
        config.SPAM?.[selectedOption] ||
        config.TEMPORAL?.[selectedOption] ||
        config.FAST?.[selectedOption] ||
        config.NEXTBLOCK?.[selectedOption] ||
        config.BLOXROUTE?.[selectedOption]) {
      console.log(
        `| ${index} | ${filePath} | ${selectedOption} | ${config.JITO[selectedOption] || config.SPAM[selectedOption] || config.TEMPORAL[selectedOption] || config.FAST[selectedOption] || config.NEXTBLOCK[selectedOption] || config.BLOXROUTE[selectedOption]} |`
      );
      index++;
    }
  }
...
async function modifyConfigs() {
  try {
    ...
    const selectedOptionIndex = parseInt(
      await askQuestion(`Select an option (1-${configOptions.length}): `)
    );
    ...
    const modifyAll = await askQuestion(
      `Do you want to modify all settings for ${selectedOption}? (y/n): `
    );
    ...
    if (modifyAll.toLowerCase() === "y") {
      ...
      for (const [filePath, config] of Object.entries(configs)) {
        ...
        if (config.SPAM?.FIXED_COMPUTE_UNIT_LIMIT) {
          await writeYamlFile(
            filePath,
            originalContents[filePath],
            "FIXED_COMPUTE_UNIT_LIMIT:",
        ...
        if (config.SPAM?.COMPUTE_UNIT_PRICE) {
          await writeYamlFile(
            filePath,
            originalContents[filePath],
            "COMPUTE_UNIT_PRICE:",
        ...
        if (config.SPAM?.MAX_COMPUTE_UNIT_PRICE) {
          await writeYamlFile(
            filePath,
            originalContents[filePath],
            "MAX_COMPUTE_UNIT_PRICE:",
      ...
      for (const [filePath, config] of Object.entries(configs)) {
        ...
        currentIndex++;
        if (config.SPAM?.FIXED_COMPUTE_UNIT_LIMIT && currentIndex === selectedIndex) {
          await writeYamlFile(
            filePath,
            originalContents[filePath],
            "FIXED_COMPUTE_UNIT_LIMIT:",
        ...
        currentIndex++;
        if (config.SPAM?.COMPUTE_UNIT_PRICE && currentIndex === selectedIndex) {
          await writeYamlFile(
            filePath,
            originalContents[filePath],
            "COMPUTE_UNIT_PRICE:",
        ...
        currentIndex++;
        if (config.SPAM?.MAX_COMPUTE_UNIT_PRICE && currentIndex === selectedIndex) {
          await writeYamlFile(
            filePath,
            originalContents[filePath],
            "MAX_COMPUTE_UNIT_PRICE:",
