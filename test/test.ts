...
const TRANSACTIONS_FILE_PATH = path.join(
  __dirname,
  CONFIG.TRANSACTIONS_FILE_NAME
);
 ...
 */
function convertBufferToReadable(obj: any, isAddress: boolean = false): any {
  ...
  if (Array.isArray(obj)) {
    return obj.map((item: any) => convertBufferToReadable(item, isAddress));
  }
 ...
 */
function decodeTransactionData(transactionData: any): any {
  ...
  if (decoded.transaction) {
    ...
    if (decoded.transaction.signature) {
      const sigBuffer = Buffer.from(
        decoded.transaction.signature.data || decoded.transaction.signature
      );
      decoded.transaction.signature = bs58.encode(sigBuffer);
    }

    if (decoded.transaction.transaction?.signatures) {
      decoded.transaction.transaction.signatures =
        decoded.transaction.transaction.signatures.map((sig: any) => {
          if (sig.type === "Buffer") {
    ...
    if (decoded.transaction.transaction?.message?.accountKeys) {
      decoded.transaction.transaction.message.accountKeys =
        decoded.transaction.transaction.message.accountKeys.map((key: any) => {
          if (key.type === "Buffer") {
    ...
    if (decoded.transaction.transaction?.message?.recentBlockhash) {
      ...
      if (bhash.type === "Buffer") {
        decoded.transaction.transaction.message.recentBlockhash = bs58.encode(
          Buffer.from(bhash.data)
        );
      }
    }
    ...
    if (decoded.transaction.transaction?.message?.addressTableLookups) {
      decoded.transaction.transaction.message.addressTableLookups =
        decoded.transaction.transaction.message.addressTableLookups.map(
          (lookup: any) => {
            const converted = { ...lookup };
            if (lookup.accountKey?.type === "Buffer") {
              converted.accountKey = bs58.encode(
                Buffer.from(lookup.accountKey.data)
              );
            }
            if (lookup.writableIndexes?.type === "Buffer") {
              converted.writableIndexes = Array.from(
                Buffer.from(lookup.writableIndexes.data)
              );
            }
            if (lookup.readonlyIndexes?.type === "Buffer") {
              converted.readonlyIndexes = Array.from(
                Buffer.from(lookup.readonlyIndexes.data)
              );
            }
            return converted;
          }
    ...
    if (decoded.transaction.transaction?.message?.instructions) {
      decoded.transaction.transaction.message.instructions =
        decoded.transaction.transaction.message.instructions.map(
          (instruction: any) => {
            const converted = { ...instruction };
            if (instruction.data?.type === "Buffer") {
              converted.data = Buffer.from(instruction.data.data).toString(
                "hex"
              );
            }
            if (instruction.accounts?.type === "Buffer") {
              converted.accounts = Array.from(
                Buffer.from(instruction.accounts.data)
              );
            }
            return converted;
          }
...
function saveTransactionToFile(transactionData: any): void {
  try {
    ...
    console.log(
      `Transaction saved to ${TRANSACTIONS_FILE_PATH} (Total: ${transactions.length})`
    );
  } catch (error) {
...
async function subscribeCommand(client: Client, args: any): Promise<void> {
  ...
  console.log(
    `Monitoring addresses: ${CONFIG.ADDRESSES_TO_MONITOR.join(", ")}`
  );
...
const client = new Client(CONFIG.GRPC_ENDPOINT, CONFIG.ACCESS_TOKEN, undefined);
